...
class Pipework(Dataset):
    def __init__(self, config):
        ...
        self.datapath = [(shape_names[i], os.path.join(self.root, shape_names[i], shape_ids[split][i])) for i
                         in range(len(shape_ids[split]))]
        print_log('The size of %s data is %d' % (split, len(self.datapath)), logger = 'Pipework')

        if self.uniform:
            self.save_path = os.path.join(self.root, 'pipework%d_%s_%dpts_fps.dat' % (self.num_category, split, self.npoints))
        else:
            self.save_path = os.path.join(self.root, 'pipework%d_%s_%dpts.dat' % (self.num_category, split, self.npoints))

        if self.process_data:
            if not os.path.exists(self.save_path):
                print_log('Processing data %s (only running in the first time)...' % self.save_path, logger = 'Pipework')
                ...
                print_log('Load processed data from %s...' % self.save_path, logger = 'Pipework')
    ...
    def __getitem__(self, index):
        ...
        return 'Pipework', 'sample', (current_points, label)